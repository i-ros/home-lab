...
## Samba design decisions

1. **Image = `dperson/samba`, not a fancier alternative (e.g. `servercontainers/samba`).** Mainstream, widely-adopted, simple flag-based share config — matches the standing preference for boring/stable over novel/complex (see Hard Rules intent: things that work now must still be fixable later without deep expertise).
2. **`/mnt/backup` share is read-only.** Purpose is restore-verification (open real files from the OneDrive pull-down backup) and general spot-check access, not editing. Read-only also avoids a real hazard: the nightly `rclone copy` treats OneDrive as the source of truth, so any local edit made through a writable share would just get silently overwritten on the next sync.
3. **`/mnt/media` is shared as one read-write root, not one share per category.** Covers `music`/`videos`/`dvr` under a single `media` share rather than a share per subfolder — one mount, and no Samba config changes needed when a new media category shows up later. Read-write (not read-only) because all of it is classified "replaceable" in Phase 1 (re-acquirable, not backed up) — worst case from an accidental delete is redoing a Plex scan, not losing something irreplaceable. This is also the intended landing spot for manual cleanup (renaming, archiving, deleting DVR cruft).
4. **One Samba stack, multiple shares** — not a separate container per share. `video-inbox` (see `todo.md` → Networking) will be added as a third `-s` block on this same `/opt/stacks/samba/compose.yml`.
5. **Container's internal `smbuser` is pinned to `USERID`/`GROUPID: "1000"`** to match the host's `ian` (owner of `/mnt/backup`/`/mnt/media`). Without this, reads work (world-readable dirs) but all writes are silently denied. Any future bind-mounted path added to this stack needs to be owned by `ian`/1000 on the host, or this stops matching.
6. **Ground truth for this stack's live config is always `docker exec samba testparm -s`.** `net view` and Explorer can both look correct while the actual permissions are wrong — this bit us once already (see `maintenance.md` → Gotchas → Samba). Confirm with `testparm -s` after any share/permission change, not just a successful `docker compose up`.
...
- **Old Immich data on the Crucial SSD — correction, it's not there.** The 2026-07-07 finding that the Crucial 500GB SATA SSD's ext4 partition was "intact" with the old Immich library was wrong (or the drive was wiped since) — confirmed 2026-08-13 the drive is empty. The planned WSL2-mount-and-rsync recovery path is dead; that drive is now going into the ITX rebuild (see `parts-inventory.md`) rather than staying set aside for migration. The only remaining source for old Immich originals is the OneDrive `immich` folder already being pulled to `/mnt/backup` by the ongoing rclone sync — Immich bring-up will need to work from that alone, no second source to reconcile against.
