...
|---|---|---|---|---|---|
| Living room | Reolink E1 Zoom (PT) | 192.168.8.106 | main HEVC 4K/20fps (`h265Preview_01_main`); sub H.264 640×360/10fps (`h264Preview_01_sub`) | `living_room` | first camera — bring-up in progress |
...
- **Reolink RTSP paths.** Assumed `h265Preview_01_main` / `h264Preview_01_sub`. If go2rtc can't connect at first boot, verify the exact paths (encoding settings can flip main between h264/h265) — this is the #1 first-boot failure.
